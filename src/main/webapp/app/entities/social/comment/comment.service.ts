...
export class CommentService {
  public resourceUrl = SERVER_API_URL + 'services/social/api/comments';
  ...
  protected convertDateFromClient(comment: IComment): IComment {
    const copy: IComment = Object.assign({}, comment, {
      createdOn: comment.createdOn != null && comment.createdOn.isValid() ? comment.createdOn.toJSON() : null
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((comment: IComment) => {
        comment.createdOn = comment.createdOn != null ? moment(comment.createdOn) : null;
      });
    }
    return res;
  }
